    ...
    <div className='flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-br from-blue-200 to-purple-400'>
     <img onClick={() => navigate('/')} src={assets.logo} alt="" className='absolute left-5 w-28 sm:left-5 sm:w-32
           ...
           <form onSubmit={onSubmitHandler} >
                   ...
                   {/*for hidding the full name 
                  input field when the user is in login state
                  ...
                  */}{state === 'Sign Up' && (
                  ...
                  <button className='w-full py-2.5 rounded-full bg-gradient-to-r from-indigo-500 to bg-purple-900 text-white'>
                    {state}
                  </button>
                </form>

                  {state === 'Sign Up' ? (
                     <p className='text-grey-400 text-center text-xs mt-4'>Already have Account? {' '}
                        <span onClick={()=>setState('Login')}
                        className='text-grey-400 cursor-pointer underline'> Login here</span>
                    </p>
                  ):(
                  <p className='text-grey-400 text-center text-xs mt-4'>
                  Don't have a Account{' '}
                  <span onClick={()=>setState('Sign Up')} className='text-grey-400 cursor-pointer underline'> Sign Up</span>
                </p>
                   )}
              </div>
    </div>
